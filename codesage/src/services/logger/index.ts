...
export class Logger {
	...
	protected getIsDev() {
		if (this._isDev === undefined) {
			...
			this._isDev = context ? context.extensionMode !== vscode.ExtensionMode.Production : (undefined as any)
		}
	...
	constructor(optionsOrLabel: LoggerOptions | string) {
		...
		const { enableDebug, label, vscodeOutputName, isDevLogger, hslColor } = options

		this.vscodeOutputChannel = vscode.window.createOutputChannel(vscodeOutputName, {
			log: true,
		})
		this.label = label
	...
	private calculateColor = (label?: string, hslConfig?: HSLColor): string | undefined => {
	...
	private logToVscodeOutputChannel: typeof this.logWithColor = (method, ...args) => {
	...
	private logToConsole: typeof this.logWithColor = (method, ...args) => {
		...
		if (isServer) {
			...
			;(console as any)[method](`%c${labelWithColor}`, `color: ${this.color}`, ...args)
		}
	}
	...
	private logWithColor = (method: "log" | "warn" | "error" | "debug", ...args: any[]) => {
